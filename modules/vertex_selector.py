...
2. Click the vertex          → it becomes the "grip" (blue box);
                               all other vertices of that feature shown
                               as hollow blue squares;
                               the full geometry is outlined in blue
3. Click the grip again      → enter MOVE mode
4. Move cursor               → live rubber-band shows the geometry
                               dynamically re-shaped with vertex at cursor
5. Click to place            → vertex moves; grip stays at new position
...
State machine
─────────────
  IDLE  ──[click vertex]──► GRIPPED ──[click same grip]──► MOVING
                │                      │                      │
           (nothing)           [click elsewhere]          [Esc / RMB]
                                     IDLE ◄──[commit click]──┘
"""
...
_C_GRIP_OTHER = QColor(0, 150, 255, 180)    # sky-blue – non-hot vertex squares
_C_GRIP_HOT   = QColor(0, 60, 220, 255)     # deep blue – hot (gripped) vertex
_C_GEOM       = QColor(0, 100, 220, 180)    # blue – geometry outline in GRIPPED
_C_GEOM_FILL  = QColor(0, 100, 220, 20)     # faint fill for polygon
...
class VertexSelector(QgsMapTool):
    ...
    def _enter_gripped(self, sv):
        ...
        for vidx, vpt in verts:
            hot = (vidx == sv.vidx)
            m = self._make_marker(
                _C_GRIP_HOT   if hot else _C_GRIP_OTHER,
        ...
        if not geom.isEmpty():
            ...
            self._geom_band = self._make_band(gt, _C_GEOM, _C_GEOM_FILL, width=2)
    ...
    def canvasMoveEvent(self, event):
        ...
        # IDLE — show yellow hover circle on the nearest vertex
    ...
    def canvasPressEvent(self, event):
        ...
        # Right-click: cancel move (if moving) or clear grip
